...
must_haves:
  truths:
    ...
    - "Keyword '!summary' triggers manager_bot action and sends placeholder response"
...
**Test 2: Manager Bot Keyword Trigger (RULE-02)**
1. Send: "!summary"
2. Expected: See in Vercel logs: `[RulesEngine] Keyword trigger matched: manager-trigger`
3. Expected: Receive WhatsApp response: "[Manager Bot] Summary feature coming in Phase 5! Your request has been logged."
4. This placeholder response confirms the manager bot trigger fired successfully
...
- Manager bot trigger sends "[Manager Bot]" placeholder response
...
- **Expected behavior:** Keyword trigger matched, manager_bot action, placeholder response received
- **Expected response:** "[Manager Bot] Summary feature coming in Phase 5! Your request has been logged."
- **Actual result:** [PASS/FAIL + details]
- **Response received:** [actual response or "none"]
- **Logs:** [relevant log excerpt]
...
- [x] RULE-02: Keyword trigger for manager bot (observable via placeholder response)
...
3. Manager bot trigger sends observable placeholder response
4. Execution logs visible in Convex dashboard
5. Verification document created with results
</verification>

<success_criteria>
- [ ] Webhook deployed to production
- [ ] Handoff keyword triggers detected (RULE-01)
- [ ] Manager bot keyword triggers detected AND placeholder response received (RULE-02)
