...
must_haves:
  truths:
    - "RLS policies use SELECT-wrapped auth.uid() for caching (or confirm existing patterns are optimal)"
...
<task type="auto">
  <name>Task 2: Review existing RLS policies for optimization</name>
  <files>none (verification only)</files>
  <action>
...
2. If any policy uses direct auth.uid() without SELECT wrapping, note it for potential optimization

If all policies already use the optimized pattern (likely true based on existing code), document that RLS is already optimized and no changes needed.

Reference: 02-RESEARCH.md Pattern 5 (RLS Policy Optimization with SELECT Wrapping).
  </action>
  <verify>RLS policies review confirms use of private.get_user_role_in_workspace() helper (optimized pattern) or identifies policies needing optimization</verify>
  <done>RLS policies reviewed - documented current state and any optimization opportunities</done>
</task>
...
- /api/contacts/by-phone: Promise.all() for notes/conversation/messages queries
- /api/conversations: Explicit columns + Promise.all() for independent queries
...
Note any policies that could be further optimized.
...
- Query count reduced: [VERIFIED - contacts: still 4 (parallelized), conversations: 4 (parallelized)]
...
2. RLS policies reviewed and documented
...
- RLS policies reviewed (documented current state)
