...
must_haves:
  truths:
    - "No select('*') wildcards exist in /api/contacts/by-phone queries"
    - "Explicit column selection for conversations and nested contact/profile relations"
    - "Query count for /api/conversations remains 4 (optimized via parallel execution)"
...
Note on query count: The 4 queries serve different data needs (conversations list, active badge count, team member assignments, tag filters). Query count cannot be reduced without merging unrelated data structures. The optimization is latency reduction via parallelization, not query count reduction.
...
Note on query count: The 4 queries remain because they serve distinct data needs. Parallelization reduces latency (wall-clock time), not query count.
  </action>
  <verify>src/app/api/conversations/route.ts uses Promise.all() for activeCount, teamMembers, contactsWithTags queries</verify>
  <done>Promise.all() pattern implemented for independent queries 2-4 (query count remains 4, optimized via parallelization)</done>
</task>
...
Note: Performance improvement will be measured after deployment to preview/production where real latency is visible. The improvement comes from parallelization, not query count reduction.
  </action>
...
8. Query count remains 4 (optimized via parallel execution, not reduction)
</verification>
...
- Query count remains 4 (optimized via parallel execution, not reduction)
