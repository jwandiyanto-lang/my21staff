...
<task type="auto">
  <name>Task 3: Integrate typing indicators and improve real-time sync</name>
  <files>src/app/(dashboard)/[workspace]/inbox/inbox-client.tsx</files>
  <action>
Update inbox-client to use typing indicators and improve real-time reliability:
...
4. Ensure real-time subscription handles all events properly
...
Update the real-time subscription to clear typing when message received:
```typescript
...
5. Verify unread count badge updates when new message arrives
  ...
  <done>Typing indicators integrated, real-time sync improved</done>
</task>
...
6. Real-time insert: New conversation appears in list when new contact messages
7. Real-time update: Conversation moves to top when new message received
</verification>
...
- [x] No chat disappearance on sync
</success_criteria>
