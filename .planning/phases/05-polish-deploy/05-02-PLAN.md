...
<task type="checkpoint:human-action" gate="blocking">
  ...
  <post-resume-verify>
    After user confirms "configured", verify webhook was actually updated by:
    1. Make a test API call to the ngrok tunnel URL to ensure it's reachable
    2. Check ngrok terminal for incoming request (confirm tunnel is working)
    If verification fails, ask user to re-check their Kapso dashboard configuration.
  </post-resume-verify>
</task>
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <post-resume-verify>
    After user confirms "received", verify the message was actually received by:
    1. Check the inbox UI to confirm the test message is visible in conversation list
    2. Verify message content matches what user sent
    3. Check terminal logs show successful webhook processing
    If verification fails, troubleshoot webhook or Convex connection before continuing.
  </post-resume-verify>
</task>
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <post-resume-verify>
    After user confirms "verified", verify the full round-trip by:
    1. Confirm sent message is visible in inbox conversation thread
    2. Confirm user received the message on WhatsApp (ask for confirmation)
    3. Check terminal/Convex logs show successful message delivery
    Document test results: receive PASS/FAIL, send PASS/FAIL for use in 05-03.
    If verification fails, identify which direction (receive/send) failed for debugging.
  </post-resume-verify>
</task>
