...
**Error handling:**
- If MCP tool returns error or empty response, document the specific error and STOP
- Do NOT proceed to Task 2 if any configuration check fails
- Common failures: auth token expired, project access revoked, config ID changed

If any configuration is missing or inactive, document the issue and halt execution.
  </action>
