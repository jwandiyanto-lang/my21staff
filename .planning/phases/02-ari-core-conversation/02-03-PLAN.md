...
3. Create helper function `getKapsoCredentials(supabase, workspaceId)`:
...
4. In `processWorkspaceMessages()`, after messages are saved (around line 298), add ARI processing:
   ```typescript
   // After: console.log(`[Webhook] Saved ${messageInserts.length} messages...`)

   // Process with ARI for each new message (async, don't await)
...
<verify>
- Webhook still returns 200 immediately (no blocking)
