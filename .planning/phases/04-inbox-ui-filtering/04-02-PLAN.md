...
must_haves:
  ...
  artifacts:
    - path: "src/components/inbox/message-bubble.tsx"
      provides: "Kapso-styled message bubbles"
      contains: "WhatsApp-like styling with background colors"
      min_lines: 120
...
<objective>
Enhance message thread with Kapso's WhatsApp-style UI: improved message bubble styling and smart auto-scroll behavior.

Purpose: Match WhatsApp Web's messaging experience with proper sender/receiver bubble differentiation and intelligent scroll handling.
Output: Message bubbles styled like Kapso's aesthetic; message thread with auto-scroll and "new messages" indicator.
</objective>
...
<task type="auto">
  <name>Enhance message-bubble with Kapso WhatsApp-style styling</name>
...
- Images: rounded-lg, max-width 100%, cursor-pointer (clickable)
- Documents: Show file icon + filename + size
- If media only (no text): Remove padding from bubble, let media be flush
...
DO NOT change component interface - only enhance visual styling within existing structure.
  </action>
  <verify>
```bash
...
```
  ...
  <done>message-bubble.tsx enhanced with WhatsApp-style backgrounds (emerald-500 for sender, white/gray-800 for receiver), characteristic rounded corners with sharp edges (rounded-tr-sm/tl-sm), 70% max-width constraint, proper alignment (sender right, receiver left), shadow, and metadata positioning</done>
</task>
