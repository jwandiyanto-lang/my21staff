...
<objective>
Fix TypeScript compilation errors in Phase 11 by extending the Contact type definition to match the actual Convex schema and creating a proper type-safe mapping for LeadPanel props.

Purpose: The backend implementation is complete (phone deduplication, lastActivityAt tracking, conversation linking), but the frontend cannot deploy due to TypeScript errors. The root cause is that `src/types/database.ts` contains an incomplete/outdated Contact type that doesn't reflect the actual Convex schema. Contacts fetched from Convex via the API route ALREADY have Sarah fields (businessType, leadStatus, leadTemperature, etc.) - the TypeScript types just don't know about them. This plan closes the gap by:
1. Extending the Contact type to match reality (what Convex actually stores)
2. Creating a safe mapping that properly types the Convex data for LeadPanel
3. Making LeadPanel gracefully handle optional fields
...
<task type="auto">
  <name>Task 1: Extend Contact type to match Convex schema</name>
  <files>src/types/database.ts</files>
  <action>
    Add an extended Contact type that includes all Sarah-specific fields as optional properties.

    CONTEXT: The project uses Convex as its database (see PROJECT.md, package.json has "convex": "^1.31.6"). The Contact type in database.ts is incomplete - it doesn't include Sarah fields that ARE stored in Convex and ARE returned by the API. This task brings the TypeScript type definition in line with what Convex actually stores.

    At the end of the file (after the existing `Contact` type alias), add:

    ```typescript
    // Extended Contact type including Sarah/Convex fields
    // These fields exist in the Convex contacts table but were missing from the original type definition
    // Data flow: Convex DB -> api.contacts.listByWorkspaceInternal -> /api/contacts route -> useContacts hook
    ...
    This creates a union type that extends the base Contact with fields that exist in Convex but were missing from the TypeScript definition. All fields are optional because not all contacts have been enriched by Sarah yet.
  </action>
  <verify>
  ...
  <done>
    ContactWithSarahFields type exported from src/types/database.ts with all Sarah-specific optional fields that match Convex schema.
  </done>
</task>
...
<task type="auto">
  ...
  <action>
    Update the contact-detail-sheet.tsx to use the extended type that matches Convex data.

    CONTEXT: Contacts come from Convex via: Convex query -> /api/contacts route -> useContacts hook -> DatabaseClient -> ContactDetailSheet. The data already has Sarah fields - we just need to tell TypeScript about them.
    ...
    This approach:
    - Properly types the Convex contact data for LeadPanel consumption
    - Converts string dates to numbers where needed (API may serialize dates as strings)
    - Gracefully handles optional fields with undefined
  ...
  <done>
    contact-detail-sheet.tsx compiles without TypeScript errors, LeadPanel receives properly typed Convex contact props.
  </done>
</task>
...
<task type="auto">
  ...
  <action>
    Update the LeadPanel component to accept contacts from any source with flexible typing.

    CONTEXT: LeadPanel can receive contacts from multiple paths (direct Convex query, API-transformed data). The interface needs to be flexible enough to handle both while maintaining type safety where possible.
    ...
    WHY: The "type instantiation excessively deep" error occurs because Convex generates deeply nested generic types. Using `as any` for the mutation function type breaks this chain while preserving runtime behavior. This is a known workaround for Convex type complexity issues.
  </action>
  <verify>
