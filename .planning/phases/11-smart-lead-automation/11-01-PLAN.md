...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/app/api/webhook/kapso/route.ts"
      provides: "Webhook wiring that passes kapso_conversation_id to mutation"
      contains: "kapso_conversation_id"
  key_links:
    - from: "src/app/api/webhook/kapso/route.ts"
      ...
      pattern: "findOrCreateContactWebhook.*kapso_conversation_id"
    - from: "src/app/api/webhook/kapso/route.ts"
...
NOTE: Task 4 updates the webhook route to pass kapso_conversation_id when calling this mutation.
  </action>
  <verify>
...
5. Verify kapso_conversation_id is accepted as an argument
  ...
  <done>
findOrCreateContactWebhook queries by phone_normalized, updates lastActivityAt, and accepts kapso_conversation_id
  </done>
</task>
...
<task type="auto">
  <name>Task 4: Wire webhook to pass kapso_conversation_id to mutation</name>
  <files>src/app/api/webhook/kapso/route.ts</files>
  <action>
Update the webhook route to link contacts to their conversations by passing the conversation_id after it's created.

The current webhook flow (processWorkspaceMessages function, lines 210-501):
1. Step 1 (lines 228-241): Creates contacts WITHOUT conversation_id
2. Step 2 (lines 244-252): Creates conversations for those contacts

The fix: After Step 2 creates the conversation, update the contact with the conversation link.

**Find this section (around lines 244-252):**
```typescript
...
**Replace with (adds conversation linking):**
```typescript
...
This ensures every contact gets linked to its conversation, enabling navigation from Database to Inbox.

WHY this approach (call mutation again) instead of a separate updateContactConversationLink mutation:
- Reuses existing mutation with new optional parameter
- No new mutation to maintain
- findOrCreateContactWebhook already handles "update if exists" logic
- Idempotent - safe to call multiple times
  </action>
  <verify>
1. Check TypeScript compiles: `npm run typecheck`
2. Verify kapso_conversation_id is passed in the webhook code:
   ```bash
   grep -n "kapso_conversation_id" src/app/api/webhook/kapso/route.ts
   ```
3. Verify the linking happens after conversation creation (around lines 244-260)
  </verify>
  <done>
Webhook passes kapso_conversation_id to findOrCreateContactWebhook mutation, linking leads to their inbox conversations
  </done>
</task>
...
# 7. Verify webhook passes kapso_conversation_id
grep -n "kapso_conversation_id" src/app/api/webhook/kapso/route.ts

# 8. Verify existing idempotency check (this already works)
...
6. Webhook passes kapso_conversation_id after conversation is created
7. All TypeScript compiles without errors
8. Convex schema deploys successfully
9. Webhook idempotency verified (kapso_message_id check exists)
</success_criteria>
