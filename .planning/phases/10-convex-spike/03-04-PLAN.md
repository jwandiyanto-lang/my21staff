...
must_haves:
  truths:
    ...
    - "Next.js API route calls Convex query via fetchQuery"
...
Output: Working HTTP action for contact lookup and Next.js API route that calls Convex via fetchQuery.
</objective>
...
<task type="auto">
  ...
  <action>
    ...
    This route calls Convex query directly via fetchQuery, maintaining identical interface to Supabase version.
  ...
  <verify>
    grep -q "fetchQuery" src/app/api/contacts/by-phone-convex/route.ts
  ...
  <done>
    by-phone-convex route exists, calls Convex getContextByPhone via fetchQuery, maintains same interface as Supabase version
  </done>
</task>
...
- Next.js API route calls Convex via fetchQuery with timing instrumentation
