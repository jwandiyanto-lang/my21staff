...
must_haves:
  truths:
    ...
    - "Users can authenticate with their existing email addresses after migration"
  artifacts:
    - path: "scripts/migrate-users-to-clerk.ts"
      provides: "User export and Clerk import logic"
      min_lines: 80
    - path: ".planning/migrations/user-id-mapping.json"
      provides: "Supabase UUID to Clerk ID mapping for data migration"
  key_links:
...
- Users can use "Forgot Password" to set credentials and log in with their existing email
