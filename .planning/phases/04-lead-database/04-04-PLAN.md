...
must_haves:
  ...
  artifacts:
    - path: "convex/leads.ts"
      ...
      exports: ["getLeadsByStatus", "getLeadsNeedingFollowUp", "getLeadStats"]
  key_links:
    - from: "convex/leads.ts"
      to: "contacts table"
      via: "ctx.db.query"
      pattern: "by_workspace"
---
...
<task type="auto">
  ...
  <action>
Add query functions to leads.ts for dashboard use. The file should already have mutations from Plan 04-05.
...
3. `getLeadStats` - Dashboard statistics (counts, temperatures, scores)
  ...
  <verify>
1. Run `npx convex dev --once` to validate queries compile
2. Grep leads.ts for "getLeadsByStatus" - should find export
3. Grep leads.ts for "getLeadsNeedingFollowUp" - should find export
4. Grep leads.ts for "getLeadStats" - should find export
  </verify>
  <done>
leads.ts exports getLeadsByStatus, getLeadsNeedingFollowUp, and getLeadStats queries in addition to existing mutations.
  </done>
</task>

<task type="checkpoint:human-verify" gate="blocking">
  <what-built>Complete lead database sync pipeline: Kapso webhook -> contact creation/update -> Sarah sync -> dashboard queries -> background sync health</what-built>
  <how-to-verify>
...
6. **Check sync health:**
   - Go to Functions -> backgroundSync:getSyncHealth
   - Should return runs array (may be empty if cron hasn't run yet)

7. **Check syncFailures table:**
   - Go to Data -> syncFailures table
   - Should be empty or have logged failures
...
- Sync health monitoring is available
  </how-to-verify>
...
2. leads.ts exports 6 functions (3 queries + 3 mutations)
3. End-to-end flow verified via checkpoint
4. Background sync and failure logging available
</verification>
...
- Sync health monitoring operational
</success_criteria>
