...
<task type="auto">
  <name>Task 3: Admin workspace switcher</name>
  <files>src/components/workspace-switcher.tsx, src/app/(dashboard)/layout.tsx</files>
  <action>
Add workspace switcher for admin users:
- Dropdown in sidebar showing all workspaces
- Admin can switch between any workspace
- Current workspace highlighted
- Same database/inbox/website features as client view

Admin sees:
- Workspace switcher dropdown (all workspaces)
- Database page (contacts for selected workspace)
- Inbox page (messages for selected workspace)
- "Add New Client" button in switcher

Client sees:
- Only their workspace (no switcher)
- Same database/inbox features
  </action>
  <verify>Admin can switch between workspaces and see full CRM</verify>
  <done>Workspace switcher for admin created</done>
</task>

<task type="auto">
  <name>Task 3b: Admin client management</name>
  <files>src/app/(dashboard)/admin/clients/*</files>
  <action>
Create client management pages (admin only):
- /admin/clients - List all clients/workspaces
- /admin/clients/new - Add new client form
- Shows client name, email, workspace, contact count, last active

Admin can:
- View all clients
- Add new client (creates user + workspace)
- See client stats
  </action>
  <verify>Admin can manage clients</verify>
  <done>Client management pages created</done>
</task>
