...
<task type="auto">
  <name>Task 2: Implement Notes tab with add/view functionality</name>
  <files>src/app/(dashboard)/[workspace]/database/contact-detail-sheet.tsx</files>
  <action>
Implement the Notes tab with:

1. **Notes list:** Display existing notes for the contact
   - Fetch via TanStack Query: GET /api/contacts/[id]/notes
   - Each note shows: content, timestamp, author
   - Sort by created_at descending (newest first)

2. **Add note form:**
   - Textarea for note content
   - Optional due date picker (for task-like notes)
   - Submit button
   - POST to /api/contacts/[id]/notes

**API route check:**
First verify if /api/contacts/[id]/notes exists. If not, create a simple implementation:
...
```
  ...
  <verify>
1. Notes tab shows empty state when no notes
2. Add note form submits and note appears in list
3. `npm run build` succeeds
  </verify>
  <done>
Notes tab displays notes list and allows adding new notes via API.
  </done>
</task>
