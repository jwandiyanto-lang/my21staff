...
must_haves:
  ...
  artifacts:
    - path: "convex/brainAnalysis.ts"
      ...
      exports: ["analyzeConversationPatterns"]
  key_links:
...
<objective>
Implement conversation pattern analysis: trending topics, objection detection, and rejection analysis.

Purpose: Help business owners understand what leads are asking about, what objections arise, and why leads go cold.

Output: Working pattern analysis that detects topics, objections, and rejection reasons using Grok.
</objective>
...
<task type="auto">
  <name>Task 1: Add pattern analysis prompts and types</name>
  <files>convex/brainAnalysis.ts</files>
  <action>
Add pattern analysis constants and types to convex/brainAnalysis.ts:

**1. System prompt for pattern analysis:**
```typescript
...
{
  "trending_topics": [
    { "topic": "...", "frequency": N, "examples": ["...", "..."] }
  ],
  "objections": [
    { "objection": "...", "frequency": N, "examples": ["...", "..."] }
  ],
  "interest_signals": [
...
**2. Insight type for structured output:**
```typescript
...
```
  ...
  <done>Pattern analysis types and prompts defined for Grok</done>
</task>
...
<task type="auto">
  ...
  <files>convex/brainAnalysis.ts</files>
  <action>
Add the main pattern analysis function:
...
**Helper query for contacts with notes:**
```typescript
...
<task type="auto">
  <name>Task 3: Add insight storage and retrieval helpers</name>
  <files>convex/brainAnalysis.ts</files>
  <action>
Add helper function to store pattern insights:
...
async function storePatternInsights(
  ...
  // Map trending topics to insights
  ...
  if (insights.length > 0) {
    ...
    console.log(`[Brain] Stored ${insights.length} insights`);
  }
}
```
...
```
  ...
  <verify>Run `npx convex dev` - insights are stored correctly in brainInsights table</verify>
  <done>Pattern analysis results stored as individual insights with confidence levels and recommendations</done>
</task>
...
3. Check brainInsights table - should have new pattern records
4. Verify confidence levels are calculated correctly
5. Verify recommendations are generated for topics and objections
</verification>
