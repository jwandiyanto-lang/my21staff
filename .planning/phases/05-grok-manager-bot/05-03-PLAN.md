...
must_haves:
  ...
  key_links:
    - from: "convex/brainAnalysis.ts"
      ...
      pattern: "api.leads.getLeadsNeedingFollowUp"
    - from: "convex/brainAnalysis.ts"
...
**1. Import the public API and add type definitions:**
```typescript
...
<task type="auto">
  ...
  <files>convex/brainAnalysis.ts</files>
  <action>
Add the main action generation function. Note: leads.ts exports regular `query` functions, so use `api.leads.X` (not `internal.leads.X`):
...
Note: The key difference is using `api.leads.getLeadsNeedingFollowUp` instead of `internal.leads.getLeadsNeedingFollowUp` because leads.ts exports regular queries, not internal queries.
  </action>
...
- Uses api.leads.X (regular queries) not internal.leads.X
</success_criteria>
