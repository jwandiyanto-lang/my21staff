...
must_haves:
  truths:
    ...
    - "User can send messages via ComposeInput and messages appear in thread"
  artifacts:
  ...
  key_links:
    ...
    - from: "ComposeInput component"
      to: "send message handler"
      via: "onSend callback"
      pattern: "handleSend|sendMessage"
---

<objective>
Integrate FilterTabs and TagFilterDropdown into inbox-client; replace popover filters with tab-based UI and verify complete filtering workflow plus message sending.

Purpose: Enable users to filter conversations using WhatsApp-style status tabs and tag multi-select with real-time updates, and confirm message sending still works.
Output: Functional inbox with status tabs, tag dropdown, combined filtering, enhanced message display, and verified send functionality.
</objective>
...
<task type="checkpoint:human-verify" gate="blocking">
  <what-built>Complete Inbox UI with status tabs, tag filtering, WhatsApp-style message bubbles, and message sending</what-built>
  <how-to-verify>
...
5. **Verify message thread (WhatsApp-style UI):**
   ...
   - [ ] **Media attachments render correctly**: If mock data has images/documents, verify they display above text in bubbles

6. **Verify message sending (INBOX-05 requirement):**
   - [ ] ComposeInput component visible at bottom of message thread
   - [ ] Type a test message in ComposeInput
   - [ ] Press Enter or click Send button
   - [ ] Message appears in thread immediately (optimistic UI)
   - [ ] Message displays with correct styling (sender bubble, right-aligned, emerald-500)
   - [ ] **ComposeInput clears after sending**
   - [ ] In dev mode: Message should appear in MOCK_CONVERSATIONS (verify send handler works)

7. **Verify auto-scroll behavior:**
   - [ ] Message thread scrolls to bottom on open (instant)
   - [ ] After sending message: Thread auto-scrolls to show new message
...
8. **Verify Active/All toggle still works:**
...
- User verifies media rendering (if mock data has media)
- **User tests message sending via ComposeInput**
...
- [ ] Message bubbles display with WhatsApp-style green/white colors
- [ ] Media attachments render correctly in bubbles (images, documents above text)
- [ ] **ComposeInput component works: User can send messages**
- [ ] **Messages appear in thread immediately after sending (optimistic UI)**
