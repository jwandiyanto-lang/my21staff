...
<task type="auto">
  <name>Create dedicated filter bar with horizontal scroll and section separation</name>
  <files>src/components/inbox/filter-tabs.tsx, src/app/(dashboard)/[workspace]/inbox/inbox-client.tsx</files>
  <action>
Enhance FilterTabs with horizontal scroll support and restructure inbox-client filter sections for clear visual hierarchy.

**Part 1: FilterTabs horizontal scroll**

**Current wrapper (filter-tabs.tsx line ~20-30):**
```tsx
...
**Part 2: inbox-client filter sections**

**Current structure (inbox-client.tsx lines ~305-361):**
```tsx
...
- FilterTabs scroll enhancement supports many status tabs
- Dedicated sections follow WhatsApp Web pattern (view toggle at top, status tabs as primary filter, search below)
- Each section has visual identity (borders, background)
- FilterTabs no longer cramped (dedicated horizontal bar)
- Still compact overall (efficient use of space)
...
- Modify button styling or badge rendering in FilterTabs
- Touch ConversationList rendering
- Touch MessageThread or InfoSidebar sections
  </action>
  <verify>
```bash
# FilterTabs: Scroll wrapper added
grep -q "overflow-x-auto" src/components/inbox/filter-tabs.tsx

# FilterTabs: Inner flex has min-w-max
grep -q "min-w-max" src/components/inbox/filter-tabs.tsx

# inbox-client: Three separate border-b sections exist
[ "$(grep -c 'border-b' src/app/\(dashboard\)/\[workspace\]/inbox/inbox-client.tsx | head -1)" -ge 3 ]

# inbox-client: FilterTabs in dedicated section (px-4 py-2 pattern)
grep -A2 -B2 "px-4 py-2.*border-b" src/app/\(dashboard\)/\[workspace\]/inbox/inbox-client.tsx | grep -q "FilterTabs"

# inbox-client: Background variations used
grep -q "bg-muted/30" src/app/\(dashboard\)/\[workspace\]/inbox/inbox-client.tsx
```
  </verify>
  <done>FilterTabs enhanced with overflow-x-auto wrapper and min-w-max for horizontal scrolling; inbox-client filter sections restructured into three dedicated areas with visual separation (border-b, background variations), FilterTabs in own horizontal bar section, improved spacing and hierarchy</done>
</task>
