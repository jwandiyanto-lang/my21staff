...
<task type="auto">
  ...
  <verify>
    - .env.local was read and all needed variables identified in .env.example
...
<task type="auto">
  ...
  <action>
    FIRST: Read planning/phases/05-polish-deploy/05-02-SUMMARY.md to get webhook test results.
    Extract the receive PASS/FAIL and send PASS/FAIL results from the summary.
    ...
    ### Webhook Test Results (from Plan 02)
    Include the actual test results from 05-02-SUMMARY.md:
    - Receive test: [result from summary]
    - Send test: [result from summary]
  ...
  <verify>
    - 05-02-SUMMARY.md was read and webhook test results extracted
    - Webhook test results from Plan 02 included in documentation
    - File exists with all sections
    - Blocking issues clearly stated
  </verify>
  <done>Deployment readiness documentation complete with webhook test results</done>
</task>
...
3. Webhook test results from 05-02-SUMMARY.md included in DEPLOYMENT-READY.md
4. Phase 5 completes v3.2 CRM milestone (local verification complete)
</verification>

<success_criteria>
- .env.example updated with only needed variables
- Deployment readiness documented
- Webhook test results from Plan 02 explicitly included
