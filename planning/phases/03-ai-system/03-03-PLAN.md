...
user_setup:
  - service: grok
    why: "Grok API for analytical AI (The Brain)"
    env_vars:
      - name: GROK_API_KEY
...
must_haves:
  truths:
    ...
    - "Analysis uses Grok for cost-effective structured output"
  artifacts:
    - path: "convex/ai/brain.ts"
      provides: "Analytical AI module (The Brain)"
      exports: ["analyzeConversation"]
    - path: "convex/ai/cost-tracker.ts"
      provides: "Cost tracking queries"
      exports: ["getWorkspaceCosts", "getConversationCost", "getRecentCalls"]
  key_links:
...
<objective>
Create "The Brain" — analytical AI module for lead scoring and conversation analysis using Grok

Purpose: The Brain runs asynchronously after The Mouth responds, analyzing conversations to score leads, update CRM data, and decide when to handoff to humans. Uses Grok API for analysis (~$5 per million tokens).
...
<task type="auto">
  <name>Task 1: Create The Brain module using Grok</name>
  <files>convex/ai/brain.ts</files>
  <action>
Create convex/ai/brain.ts with analyzeConversation function using Grok API:
...
- Grok uses OpenAI-compatible format (no special SDK needed)
- Extract JSON from response (Grok may wrap in markdown code blocks)
  </action>
  ...
  <done>convex/ai/brain.ts exports analyzeConversation internalAction using Grok API with helper mutations for updating contacts, conversations, and logging usage.</done>
</task>
...
To verify queries work correctly:
1. Open Convex Dashboard (https://dashboard.convex.dev)
2. Navigate to Functions tab
3. Find `ai/cost-tracker:getWorkspaceCosts`
4. Test with a valid workspace ID and timestamp range (e.g., fromTimestamp: 0, toTimestamp: Date.now())
5. Confirm it returns a CostSummary object (even if empty initially)
  </verify>
  <done>convex/ai/cost-tracker.ts exports queries for workspace costs, conversation costs, and recent AI calls. Queries tested in Convex dashboard.</done>
</task>
...
4. Query test: getWorkspaceCosts returns valid CostSummary in Convex dashboard
</verification>

<success_criteria>
- convex/ai/brain.ts exports analyzeConversation internalAction using Grok API
...
- Grok model ID is "grok-beta"
- No external SDK dependency (uses fetch API)
- getWorkspaceCosts query tested and returns valid response
</success_criteria>
