...
must_haves:
  ...
  key_links:
    ...
    - from: "src/app/api/contacts/import/route.ts"
      ...
      pattern: "from '@/lib/phone/normalize'"
---
...
<task type="auto">
  ...
  <action>
**PREREQUISITE CHECK:** Verify normalizePhone helper exists before using it:
```bash
ls src/lib/phone/normalize.ts && echo "EXISTS" || echo "MISSING"
```

**If MISSING**, create `src/lib/phone/normalize.ts`:
```typescript
...
```
  ...
  <verify>
- `ls src/lib/phone/normalize.ts` confirms helper exists
- `npm run build` passes
- `grep "supabase" src/app/api/contacts/export/route.ts src/app/api/contacts/import/route.ts` returns nothing
  </verify>
  <done>
...
```
  ...
  <verify>
- `npm run build` passes
- `grep "supabase" src/app/api/contacts/merge/ src/app/api/contacts/*/notes/ src/app/api/notes/ src/app/api/leads/` returns nothing
  </verify>
  <done>
