...
must_haves:
  truths:
    ...
    - "Eagle workspace has meta_access_token encrypted and stored"
...
Important: meta_access_token should be stored encrypted. Check if encryption helper exists.
If encryption is needed, use the ENCRYPTION_KEY env var pattern from src/lib/kapso/client.ts.

If the mutation already exists, verify it works correctly.
If not, create it following existing patterns in convex/workspaces.ts.

Do NOT include actual API keys in code - they will be set via Convex dashboard or CLI.
  </action>
  <verify>
Check convex/workspaces.ts exports updateKapsoCredentials or similar mutation.
Verify TypeScript compiles: `cd /home/jfransisco/Desktop/21/my21staff && npx convex dev --once`
  </verify>
  <done>Mutation exists to update Kapso credentials on workspace.</done>
</task>
